...
const Viewbookdetails = () => {
  ...
  const fetch = async () => {
    const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/books/${id}`)
  ...
  const handleCart = async () => {
    ...
    const response = await axios.post(`${process.env.REACT_APP_BASE_URL}/customer/addtocart`, {
        ...
        data && (
          <div className='px-4 sm:px-12 py-8 bg-zinc-900 flex flex-col sm:flex-row'>
            <div className='bg-zinc-800 rounded p-4 w-full sm:w-3/6 flex justify-center flex-col sm:flex-row items-center sm:items-start'>
              <img src={data.cover_photo} alt="" className='h-[30vh] sm:h-[70vh]' />
              {isLoggedIn && role === "user" &&
                <div className='flex flex-row sm:flex-col   p-2'>
                  <button className='bg-white text-blue-300 rounded-full text-xl p-2 w-auto' onClick={handleCart}><FaCartShopping /></button>
                </div>
              }
            ...
            <div className='p-4 w-full sm:w-3/6'>
              ...
              <p className='text-zinc-100 mt-1 text-xs  sm:text-sm'>{data.description}</p>
